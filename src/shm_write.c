...
int8_t shm_write(struct SharedMemoryInfo *const shmInfo, const void *const data, const size_t data_byte_size, const size_t indent_bytes)
{
	...
	if ((data_byte_size + indent_bytes + shmInfo->writed_bytes) > shmInfo->shmByteSize)
	{
		is_resize = true;
		size_t old_byte_size = 0;

		old_byte_size = shmInfo->shmByteSize;
		shmInfo->shmByteSize = data_byte_size + indent_bytes + shmInfo->writed_bytes;
	...
	memcpy(shmInfo->pMappedShmAddr + shmInfo->writed_bytes + indent_bytes, data, data_byte_size);
	if (is_resize == true)
	{
